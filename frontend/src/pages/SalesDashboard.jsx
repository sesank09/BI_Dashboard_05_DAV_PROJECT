  ...
  if (error || !data) {
    return (
      <div className="flex flex-col items-center justify-center h-64 text-slate-600 gap-3 bg-white rounded-xl border border-slate-200 p-6">
        <p className="text-sm font-medium text-red-600">{error || 'No sales data available.'}</p>
        <button
          onClick={fetchData}
              ...
              <tbody className="divide-y divide-slate-100 font-medium">
                {data.sales_rep_leaderboard?.map((rep, idx) => (
                  <tr key={idx} className="hover:bg-slate-50">
                    ...
                    <td className="py-2.5 px-3 text-right text-emerald-700 font-bold">₹{Number(rep.revenue).toLocaleString('en-IN')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
